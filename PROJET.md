...
d'utilisation, le certificat public, et les scripts qui publient une version (`publier.ps1`,
`Publier.bat`, `Publier-Beta.bat`). **Le code du logiciel ne vit pas ici** — il est dans
`Press-Start`, dépôt privé. Ici, on publie ce qui est déjà compilé.
...
- **`garde-fous-depot` avant chaque commit**, sans exception. Sur un dépôt public, l'erreur ne se
  rattrape pas.
...
`garde-fous-depot`. Les étapes amont valent quand le chantier touche à la manière de publier, pas
quand il corrige une phrase.

## Publier une version

Les scripts tournent **sur la station Windows**, jamais depuis le Mac : ils prennent un dossier déjà
compilé par `app/build.py` et le déposent en release. Lire `publier.ps1` avant de le lancer, et
